...
class GCONConv(MessagePassing):
    ...
    def __init__(
        ...
        self.activation_att1 = activation_resolver(activation_att1)
        self.activation_att2 = activation_resolver(activation_att2)
...
@register_layer('gcon')
class GCONLayer(nn.Module):
    """HybridConv layer"""

    def __init__(self, dim_in, dim_out, channel_list, combine_fn, num_heads, activation_att1, activation_att2, activation, depth_mlp, skip, add_self_loops, norm, filter_norm_dim, bias, **kwargs):
        super().__init__()
        self.model = GCONConv(
            dim_in,
            dim_out,
            ...
            num_heads=num_heads,
            activation_att1=activation_att1,
            activation_att2=activation_att2,
            activation=activation,
            depth_mlp=depth_mlp,
            skip=skip,
            add_self_loops=add_self_loops,
            norm=norm,
            filter_norm_dim=filter_norm_dim,
            bias=bias,
            **kwargs
        )
